...
/**
 * Pied de page en pleine hauteur — 60 % de l'écran, plus 200 px — sur fond
 * pétrole.
 *
 * La réserve de 200 px en haut accueille le tapis volant, remonté à cheval sur
 * la section précédente — le pied de page ne borne donc pas ce qui déborde vers
 * le haut. Vers le bas en revanche, c'est `RugDivider` qui referme : sans quoi
 * la boîte du tapis allongeait le document sous le pied de page et y ouvrait
 * une bande de fond blush.
 *
 * `FooterReveal` remplace la balise `footer` : c'est lui qui pilote la mise en
 * place au défilement (échelle et coins arrondis).
 */
export function SiteFooter({ settings }: { settings: SETTINGS_QUERY_RESULT | null }) {
  return (
    <FooterReveal className="relative flex min-h-[calc(60svh+200px)] flex-col justify-end bg-petrole px-sp-4 pb-sp-6 pt-[200px] text-blush md:px-sp-5">
      <RugDivider />
